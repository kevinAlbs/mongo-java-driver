...
@IgnoreIf({ getCredentialList().isEmpty() || getCredentialList().get(0).getAuthenticationMechanism() != SCRAM_SHA_1 })
class ScramAuthenticationSpecification extends Specification {
    ...
    def 'should throw MongoSecurityException when authentication fails'() {
        given:
        def connection = createConnection(async, createScramSha1Credential('wrongUserName', '$external', 'wrongPassword'.toCharArray()))
