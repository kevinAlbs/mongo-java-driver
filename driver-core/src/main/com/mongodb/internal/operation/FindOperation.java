...
import static com.mongodb.internal.operation.CommandOperationHelper.CommandTransformer;
import static com.mongodb.internal.operation.CommandOperationHelper.executeWrappedCommandProtocol;
import static com.mongodb.internal.operation.CommandOperationHelper.executeWrappedCommandProtocolAsync;
    ...
    private CommandTransformer<BsonDocument, BatchCursor<T>> transformer(final ConnectionSource source,
        ...
        return new CommandTransformer<BsonDocument, BatchCursor<T>>() {
            ...
            public BatchCursor<T> apply(final BsonDocument result, final ServerAddress serverAddress) {
                QueryResult<T> queryResult = cursorDocumentToQueryResult(result.getDocument("cursor"), serverAddress);
                return new QueryBatchCursor<T>(queryResult, limit, batchSize, getMaxTimeForCursor(), decoder, source, connection);
            }
        };
    }
