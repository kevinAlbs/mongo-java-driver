...
public final class ClientMetadataHelper {
    ...
    // The driver name is stored in a class generated by a build plugin, so it has to be accessed dynamically here
    static String getDriverName() {
        return getFieldValue(getField(getMongoDriverVersionClass(), "NAME"));
    }

    // The driver version is stored in a class generated by a build plugin, so it has to be accessed dynamically here
    static String getDriverVersion() {
        return getFieldValue(getField(getMongoDriverVersionClass(), "VERSION"));
    }

    private static Class<?> getMongoDriverVersionClass() {
        try {
            return Class.forName("com.mongodb.internal.build.MongoDriverVersion");
        } catch (ClassNotFoundException e) {
            throw new MongoInternalException("Unable to load generated MongoDriverVersion class", e);
        }
    }

    private static Field getField(final Class<?> mongoDriverVersionClass, final String fieldName) {
        try {
            return mongoDriverVersionClass.getDeclaredField(fieldName);
        } catch (NoSuchFieldException e) {
            throw new MongoInternalException("Static VERSION field of generated MongoDriverVersion class does not exist");
        }
    }

    private static String getFieldValue(final Field field) {
        try {
            return (String) field.get(null);
        } catch (IllegalAccessException e) {
            throw new MongoInternalException("Unable to access value of static VERSION field of generated MongoDriverVersion class");
        }
    }
